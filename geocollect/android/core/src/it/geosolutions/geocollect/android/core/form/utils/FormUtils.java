...
public class FormUtils {
    ...
    /**
     * Provides the drawable for the icon class in <MissionTemplate> If the correct drawable cannot be found, returns a question mark icon
     * 
     * @param iconCls
     * @return
     */
    ...
    public static void resizeFotosToMax(final Context context, final String feature_id,
        ...
        File folder = new File(MapFilesProvider.getEnvironmentDirPath(context)
                + "/geocollect/media/" + feature_id);
        folder.mkdirs();
        ...
        for (int i = 0; i < listOfFiles.length; i++) {
            ...
            try {
                ...
                final String newfileName = origFile.getName().substring(0,
                        origFile.getName().lastIndexOf("."))
                        + "_resized";
                final String extension = origFile.getName().substring(
                        origFile.getName().lastIndexOf(".") + 1);

                final String newFile = origFile.getPath().substring(0,
                        origFile.getPath().lastIndexOf("/") + 1)
    ...
    /**
     * calculates the samplesize (the scale factor 1 / n) to subsample the image Note : A power of two value is calculated because the decoder uses a
     * final value by rounding down to the nearest power of two, as per the inSampleSize documentation.
     * 
     * @param options
     * @param maxSize
     * @return
    ...
    /**
     * Return a list of Strings representing Uris for the media TODO: Filter media based on feature.id
     * 
     * @return
     */
    public static String[] getPhotoUriStrings(final Context context, final String feature_id) {
        ...
        File folder = new File(MapFilesProvider.getEnvironmentDirPath(context)
                + "/geocollect/media/" + feature_id);// TODO Parametrize this
        folder.mkdirs();
        File[] listOfFiles = folder.listFiles();
